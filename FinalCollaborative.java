...
public class FinalCollaborative extends Configured implements Tool {
	...
	public static class Job2Mapper extends Mapper<LongWritable, Text, Text, LongWritable>{
		@Override
		protected void map(LongWritable key, Text value,Context context)//for each user
				throws IOException, InterruptedException {
			String[] split = value.toString().split("[\t,]");//userid,movieid,rating
			for (int i = 1; i < split.length; i++) {//movieid:rating
				String[] movieidrating = split[i].split(":");//movieid:rating
				for (int j = 1; j < split.length; j++) {//movieid:rating
					String[] movieidrating1 = split[j].split(":");//movieid:rating
					String pairs = movieidrating[0]+":"+movieidrating1[0];//movieid:movieid pairs
